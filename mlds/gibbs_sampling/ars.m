...
% pr = ( exp(m*S(1)+b ) - 0 ) / m; % integrating in from -infinity
upperHull(1).m = m;
upperHull(1).b = b;
% upperHull(1).pr = pr;
...
%     pr1 = ( exp(m1*ix+b1) - exp(m1*S(li)+b1) ) / m1;
	i = length(upperHull)+1;
	upperHull(i).m = m1;
	upperHull(i).b = b1;
% 	upperHull(i).pr = pr1;
...
%     pr2 = ( exp(m2*S(li+1)+b2) - exp(m2*ix+b2) ) / m2;
	i = length(upperHull)+1;
	upperHull(i).m = m2;
	upperHull(i).b = b2;
% 	upperHull(i).pr = pr2;
...
% pr = ( exp(m*S(end)+b) - exp(m*S(end-1)+b) ) / m;
i = length(upperHull)+1;
upperHull(i).m = m;
upperHull(i).b = b;
% upperHull(i).pr = pr;
...
if 0 % could cause overflow
    for i = 1:length(upperHull)
        u = upperHull(i);
        upperHull(i).pr = (exp(u.m * u.right + u.b) - exp(u.m * u.left + u.b) ) / u.m;
    end
else
    candidates = [upperHull.m] .* [upperHull.right] + [upperHull.b];
    candidates = [candidates, [upperHull.m] .* [upperHull.left] + [upperHull.b]];
    candidates(isinf(candidates)) = [];
    const = -max(candidates);
    for i = 1:length(upperHull)
        u = upperHull(i);
        upperHull(i).pr = (exp(u.m * u.right + u.b + const) - ...
            exp(u.m * u.left + u.b + const) ) / u.m;
    end

end

Z = sum([upperHull(:).pr]);
if Z == 0 || isinf(Z) || isnan(Z)
    error('The sum of upper hull probabilities is not valid.');
end
